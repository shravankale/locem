...
from loss_triple import tripletLoss
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    criterion_yolo = yoloLoss(feature_size=S, num_bboxes=B, num_classes=C, lambda_coord=5.0, lambda_noobj=0.5)
    criterion_triplet = tripletLoss(feature_size=S, num_bboxes=B, num_classes=C, lambda_coord=5.0, lambda_noobj=0.5,beta=64,gamma=1)
...
def train(train_loader, model, criterion_yolo, criterion_triplet, optimizer, epoch, args, writer):
    ...
    loss_yolo_class_m = AverageMeter('Yolo_Class_Loss',':.4e')
    loss_yolo_xywh_m = AverageMeter('Yolo_XYWH_Loss',':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses,loss_yolo_m,loss_triplet_m,loss_yolo_class_m,loss_yolo_xywh_m, top1, top5],
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        output_yolo, output_triplet = model(images)
        output_yolo = output_yolo.view(-1, S, S, 5 * B + C)
        output_triplet = output_triplet.view(-1, S, S, B * 5 + beta)

        loss_yolo,loss_class,loss_xywh = criterion_yolo(output_yolo,target)
        loss_triplet = criterion_triplet(output_triplet,target)
        loss = loss_yolo + loss_triplet
    ...
    writer.add_scalar('Train/Yolo_Loss', loss_yolo_m.avg, epoch)
    writer.add_scalar('Train/Triplet_Loss', loss_triplet_m.avg, epoch)
    writer.add_scalar('Train/Yolo_Class_Loss', loss_yolo_class_m.avg, epoch)
    writer.add_scalar('Train/Yolo_xywh_Loss', loss_yolo_xywh_m.avg, epoch)
...
def validate(val_loader, model, criterion_yolo, criterion_triplet, args, writer, epoch, mini_display=False):
    ...
    with torch.no_grad():
        ...
        if epoch is not None:
            ...
            writer.add_scalar('Validate/Yolo_Loss', loss_yolo_m.avg, epoch)
            writer.add_scalar('Validate/Triplet_Loss', loss_triplet_m.avg, epoch)
            writer.add_scalar('Validate/Yolo_Class_Loss', loss_yolo_class_m.avg, epoch)
            writer.add_scalar('Validate/Yolo_xywh_Loss', loss_yolo_xywh_m.avg, epoch)
