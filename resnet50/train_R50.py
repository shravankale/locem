...
from genINV_R50_v2 import ImageNetVID
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader = DataLoader(gen_train,batch_size=args.batch_size,num_workers=args.workers,shuffle=True)
    ...
    val_loader = DataLoader(gen_val,batch_size=256)
    ...
    if args.evaluate:
        ...
        val_loader = DataLoader(gen_val,batch_size=256)
        acc1,avg_metrics_epoch_val = validate(val_loader, model, criterion, args, writer, epoch=None,saveEmbeddings='train')
...
def train(train_loader, model, criterion, optimizer, epoch, args, writer):
    ...
    for i, (images,target,filename,idx) in enumerate(train_loader):
...
def validate(val_loader, model, criterion, args, writer, epoch, mini_display=False,saveEmbeddings=None):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    ...
    with torch.no_grad():
        ...
        for i, (images,target,filename,idx) in enumerate(val_loader):
