...
class ImageNetVID(data.Dataset):
    ...
    def preprocessInputImage(self,img,network_dim):
    ...
    def __getitem__(self,idx):
        ...
        sample_img = self.preprocessInputImage(sample_img,self.network_dim)
        sample_img = self.to_tensor(sample_img)
        sample_class = sample_class[0][0]


        ids = self.unique_ids[(self.unique_ids.cat_code==sample.cat_code) & (self.unique_ids.snip_id==sample.snip_id) & (self.unique_ids.trackid==sample.trackid)].index.array
        ids = int(ids)

        return [sample_img,sample_class,sample.file,ids]
